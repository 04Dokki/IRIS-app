...
static uint8_t *last_alert_buf = NULL;
static size_t last_alert_len = 0;
static portMUX_TYPE last_alert_mux = portMUX_INITIALIZER_UNLOCKED;
...
void captureLastAlertFrame() {
  ...
  if (copy) {
    ...
    portENTER_CRITICAL(&last_alert_mux);
    uint8_t *old_buf = last_alert_buf;
    last_alert_buf = copy;
    last_alert_len = jpg_len;
    portEXIT_CRITICAL(&last_alert_mux);
    if (old_buf) {
      free(old_buf);
    }
  }
...
static esp_err_t last_capture_handler(httpd_req_t *req) {
  portENTER_CRITICAL(&last_alert_mux);
  uint8_t *buf = last_alert_buf;
  size_t len = last_alert_len;
  portEXIT_CRITICAL(&last_alert_mux);
...
void startCameraServer() {
  ...
  httpd_uri_t last_capture_uri = {
    .uri = "/capture/last.jpg",
    .method = HTTP_GET,
    .handler = last_capture_handler,
    .user_ctx = NULL
#ifdef CONFIG_HTTPD_WS_SUPPORT
    ,
    .is_websocket = true,
  ...
  if (httpd_start(&camera_httpd, &config) == ESP_OK) {
    ...
    httpd_register_uri_handler(camera_httpd, &last_capture_uri);
